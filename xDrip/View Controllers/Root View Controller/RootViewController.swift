...
final class RootViewController: UIViewController, ObservableObject {
    ...
    private func updateDataSourceInfo(animate: Bool = false) {
        ...
        if let cgmTransmitter = self.bluetoothPeripheralManager?.getCGMTransmitter() {
            if let deviceName = cgmTransmitter.getDeviceName(), let sensorName = cgmTransmitter.getSensorName() {
                let attributedString = NSMutableAttributedString()

                attributedString.append(NSAttributedString(string: deviceName, attributes: [.foregroundColor: UIColor.systemGreen]))
                attributedString.append(NSAttributedString(string: " + ", attributes: [.foregroundColor: ConstantsHomeView.sensorProgressNormalTextColor]))
                attributedString.append(NSAttributedString(string: sensorName, attributes: [.foregroundColor: UIColor.systemGreen]))
                dataSourceLabelOutlet.attributedText = attributedString

                UserDefaults.standard.activeSensorDescription = "\(deviceName) + \(sensorName)"
            } else if let deviceName = cgmTransmitter.getDeviceName() {
                UserDefaults.standard.activeSensorDescription = "\(deviceName)"
                
                dataSourceLabelOutlet.text = UserDefaults.standard.activeSensorDescription
                dataSourceLabelOutlet.textColor = .systemGreen
