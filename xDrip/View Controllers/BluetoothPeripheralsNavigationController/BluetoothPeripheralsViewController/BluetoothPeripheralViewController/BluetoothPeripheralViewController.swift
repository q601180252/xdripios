...
class BluetoothPeripheralViewController: UIViewController {
    ...
    public func connectButtonHandler(_ askUser: Bool = true) {
        ...
        if let bluetoothPeripheral = bluetoothPeripheral {
            ...
            if bluetoothPeripheral.blePeripheral.shouldconnect {
                ...
                // disconnect
                setShouldConnectToFalse(for: bluetoothPeripheral, askUser: askUser)
...
extension BluetoothPeripheralViewController {
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        ...
        if segue.identifier == "showOTA" {
            let firmware = bluetoothPeripheral?.blePeripheral.bubble?.firmware
            let address = bluetoothPeripheral?.blePeripheral.address
            let fotaFile = sender as! FotaFile
            
            bluetoothPeripheralManager?.getCGMTransmitter()?.startOTA()
            
            DispatchQueue.main.asyncAfter(deadline: .now() + .seconds(1)) { [weak self] in
                self?.bluetoothPeripheralManager?.getBluetoothTransmitters().first?.disconnect()
            }
            
            if let targetViewController = segue.destination as? BleDeviceViewController {
                targetViewController.firmware = firmware
                targetViewController.address = address
                targetViewController.previousViewController = self
                targetViewController.fotaFile = fotaFile
            }
        } else if segue.identifier == "showBatteryLog" {
            if let targetViewController = segue.destination as? LogsViewController {
                targetViewController.coreDataManager = coreDataManager
            }
        }
    }
