...
final class BubbleCrc {
    ...
    public static func LibreCrc(data:inout Data, headerOffset:Int, libreSensorType: LibreSensorType?) -> Bool {
        ...
        return BubbleCrc.hasValidCrc16InFirstTwoBytes([UInt8](data.subdata(in: headerRange))) &&
        BubbleCrc.hasValidCrc16InFirstTwoBytes([UInt8](data.subdata(in: bodyRange))) &&
        BubbleCrc.hasValidCrc16InFirstTwoBytes([UInt8](data.subdata(in: footerRange)))
    }
    ...
    private static func hasValidCrc16InFirstTwoBytes(_ bytes: [UInt8]) -> Bool {
        ...
        let calculatedCrc = BubbleCrc.crc16(Array(bytes.dropFirst(2)), seed: 0xffff)
